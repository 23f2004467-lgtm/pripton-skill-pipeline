...
Each section is sent to the language model independently and in parallel via `asyncio.gather`, bounded by a semaphore so at most a few sections call the model at once. This keeps a many-section document from bursting every request simultaneously and tripping provider rate limits; the semaphore reduces burstiness, while the transport layer's exponential-backoff retry on HTTP 429 remains the actual rate-limit backstop. The model is asked, via tool-use / function-calling (Groq's OpenAI-compatible API), to return a list of topics with five fields each: id (a slug), name, short description, category, and difficulty (beginner / intermediate / advanced). The tool-use mechanism is materially more reliable than asking the model for JSON in free-form text — the response is validated against a declared schema by the API itself, then re-validated against the corresponding Pydantic model on our side.

The document body is inserted into the prompt as delimited, untrusted data: the system prompt instructs the model to treat document content as material to analyze and never to act on instructions embedded in it (a basic prompt-injection mitigation). For curated learning documents this threat is low-severity; the delimiting plus tool-use output enforcement is cheap defense-in-depth, not a hard boundary against a determined attacker.
...
We then generalized the lesson rather than patching the single instance. The real-graph test still uses `FakeLLMClient`, so it does not exercise how `GroqLLMClient` parses a *real* Groq response — the same "green in tests, never ran for real" boundary, one layer down. `tests/test_groq_response_parsing.py` closes it: it constructs a genuine `groq.types.chat.ChatCompletion` from the SDK's own types and runs `GroqLLMClient.call` against it (network mocked), covering JSON-string tool arguments, text-only responses, and malformed arguments, so a response-shape change fails a test instead of the live path. Three further pieces of hardening landed in the same spirit: extraction is now bounded by a semaphore so a many-section document does not burst past rate limits; the document body is delimited as untrusted data in the prompt as a basic prompt-injection mitigation; and prompts load relative to the package (`Path(__file__).parent`) rather than the working directory, so the tool runs from any directory.
...
1. **Semantic-correctness evaluation.** This is the single biggest gap: none of our structural validations catch a topic that's plausible but wrong, or a prerequisite relationship pointing the wrong direction. The anchor would be a small hand-labeled relationship set scored by precision/recall; an LLM-as-judge (run with a *different* model than the extractor) is a cheap pre-filter only after it's calibrated against that labeled set — leading with the judge alone would just move the "confidently wrong" failure up a level. The cost rises per run; the quality improvement is substantial.
